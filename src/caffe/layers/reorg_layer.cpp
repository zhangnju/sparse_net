...
void ReorgLayer<Dtype>::Reshape(const vector<Blob<Dtype> *> &bottom,
                                const vector<Blob<Dtype> *> &top) {
  top[0]->Reshape(batch_num_, reorged_channels_, reorged_height_, reorged_width_);
}
...
void ReorgLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype> *> &bottom,
  ...
  reorg_cpu(bottom_data, width_, height_, channels_, batch_num_, stride_, reverse_, top_data);
}
...
void ReorgLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype> *> &top, const vector<bool> &propagate_down,
  ...
  reorg_cpu(top_diff, width_, height_, channels_, batch_num_, stride_, !reverse_, bottom_diff);
}
